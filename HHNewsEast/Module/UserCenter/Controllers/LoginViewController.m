...
- (void)viewDidLoad
{
    ...
    self.dataMutableArray=[LoginItemModel loginItemsArray];
...
-(HHView *)footView{
    if (nil==_footView) {
        ...
        HHButton *noAccountButton=[HHButton buttonWithType:UIButtonTypeCustom frame:CGRectMake(50,  CGRectGetHeight(_footView.bounds)-40,78,20) titleColor:[UIColor darkGrayColor] titleSize:16];
        [noAccountButton addTarget:self action:@selector(registerButtonPressed) forControlEvents:UIControlEventTouchUpInside];
            // [registerButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        NSString *str0=@"没有账号?";
        [noAccountButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [noAccountButton setTitle:str0 forState:UIControlStateNormal];
        noAccountButton.titleLabel.textAlignment=NSTextAlignmentRight;
        [noAccountButton setContentHorizontalAlignment:UIControlContentHorizontalAlignmentRight];
        [_footView addSubview:noAccountButton];
        
        HHButton *registerButton=[HHButton buttonWithType:UIButtonTypeCustom frame:CGRectMake(noAccountButton.frame.origin.x+noAccountButton.frame.size.width,  CGRectGetHeight(_footView.bounds)-40, 140,20) titleColor:[UIColor darkGrayColor] titleSize:16];
        [registerButton addTarget:self action:@selector(registerButtonPressed) forControlEvents:UIControlEventTouchUpInside];
            // [registerButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        NSString *str=@"赶紧去注册账号吧!";
        [registerButton setTitleColor:[UIColor red:181 green:6 blue:16 alpha:1] forState:UIControlStateNormal];
        [registerButton setTitle:str forState:UIControlStateNormal];
...
-(void)loginButtonPressed{
    ...
    if ([NSString IsNullOrEmptyString:userName]||[NSString IsNullOrEmptyString:userPwd]) {
        [self showAlertView:@"请填写完成信息"];
    }else{
        self.op=[[HHNetWorkEngine sharedHHNetWorkEngine] userLoginWithUserName:userName userPwd:userPwd OnCompletionHandler:^(HHResponseResult *responseResult) {
...
-(void)registerButtonPressed{
    RegisterViewController *registerVeiwController=[[RegisterViewController alloc]  init];
    registerVeiwController.navigationItem.title=@"注册华人频道";
    [self.navigationController pushViewController:registerVeiwController animated:YES];
}
