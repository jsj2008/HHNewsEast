...
-(void)onInitDataWithArry:(NSMutableArray *)titleArray{
    ...
    for (NSInteger i = 0 ; i < titleArray.count; i++)
    {
        ...
        ArticleClassModel *classModel=[titleArray objectAtIndex:i];
        [button setTitle:classModel.articleClassName forState:UIControlStateNormal];
        ...
        CGSize size = [self string:classModel.articleClassName boundingRectWithfont:[UIFont systemFontOfSize:kClassMenuTitleButtonFontSize] maxTextSize:CGSizeMake(MAXFLOAT, 25)];
        ...
        button.frame = CGRectMake(width,(CGRectGetHeight(self.bounds)-__gOffY-kClassMenuButtonHeight)/2+__gOffY, size.width, kClassMenuButtonHeight);
        ...
        if (i==(titleArray.count-1)) {// 吧最后一个button 给加上
            ...
            width=width<__gScreenWidth?__gScreenWidth:width;
        }
    ...
    leftButton.frame=CGRectMake(0, __gOffY, kClassMenuLeftButtonWidth, __gTopViewHeight-__gOffY);
    [leftButton setBackgroundImage:[UIImage imageNamed:@"btn_ClassViewLeftButton"] forState:UIControlStateNormal];
    leftButton.tag=kButtonTagStart-1;
...
-(void)selectClassMenuAtIndex:(NSInteger)index
{
    ...
    {  UIButton *currentSelectedButton=(UIButton *)[self viewWithTag:(kButtonTagStart+index)];
        ...
        ArticleClassModel *classModel=[_classDataArry objectAtIndex:index];
        _selectIndex =  index;
        ...
        {
            [_menuDelegate classMenuSelectIndexChanded:_selectIndex classID:classModel.articleClassID];
        }
        
        
        if (lineRC.origin.x - self.contentOffset.x > CGRectGetWidth(self.bounds) * 2  / 3)
        {
        ...
        else if ( lineRC.origin.x - self.contentOffset.x < CGRectGetWidth(self.bounds) / 3)
        {
            NSInteger index = _selectIndex;
            if (_selectIndex - 2 >= 0)
            {
