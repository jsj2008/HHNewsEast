...
-(void)onInitDataWithArry:(NSMutableArray *)arry column:(NSInteger)colum{
   CGSize size=[HHShareItemView shareItemViewSize];//default is 60
    ...
    frame.size.height=kHHShareContentViewTopViewHeight +(arry.count/colum+((arry.count%colum)>0?1:0))*(size.height+paddingHeihght)+paddingHeihght;//paddingheiht = cout+1
    self.frame=frame;
}
